...
#            k_target = np.zeros(10,dtype=np.int)
#            k_target[i] = 1
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        load = np.loadtxt(data_path,skiprows=3,
           dtype=np.dtype([('year',np.int),('month',np.int),('precipitation',(np.float,31))]))
        # filter out all missing datapoints and flatten to a vector
        precipitation = load['precipitation'][load['precipitation']>-0.1].flatten()
        # normalise data to zero mean, unit standard deviation
        norm_data = (precipitation-np.mean(precipitation))/(np.std(precipitation))
        # convert from flat sequence to windowed data
        window_data = np.array([norm_data[i:i+window_size] for i in range(len(norm_data)-window_size)])
        # inputs are first (window_size - 1) entries in windows
        inputs = window_data[:,:-1].astype(np.float32)
        # targets are last entry in windows
        targets = window_data[:,-1].astype(np.float32)
